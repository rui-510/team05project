...
def connect(auth):
    ...
    # 接続者数の更新（全員向けに送信）
    # 入室通知をする(alert = true)
    emit('count_update', {'user_count': user_count, 'good_count': good_count, 'name': user_name, 'members': members, 'alert': True},  room=room_id)
...
def disconnect():
    ...
    emit('count_update', {'user_count': user_count, 'good_count': good_count, 'members': members, 'alert': False}, room=room_id)
