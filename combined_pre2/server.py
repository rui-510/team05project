...
def callback():
    ...
    sub_hash = generate_password_hash(sub)

    check_id = db.execute("SELECT id FROM users WHERE username = ?", sub_hash)

    if check_id == []:
        new_user = db.execute("INSERT INTO users (username, is_line_account) VALUES (?, 1)", sub_hash) # 新規登録の場合
